...
% words_with_4_repeated_2(+N:integer, +Alphabet:list, -Word:list)
% Generates words of length N with exactly 4 letters, each repeated twice, from the given Alphabet.
words_with_4_repeated_2(N, Alphabet, Word) :-
    length(Word, N),
    % Select four letters to be repeated twice
    select(A, Alphabet, Rest1),
    select(B, Rest1, Rest2),
    select(C, Rest2, Rest3),
    select(D, Rest3, Rest4),
    A \= B, A \= C, A \= D, B \= C, B \= D, C \= D,
    % Form a list with the four letters, each repeated twice
    append([A, A, B, B, C, C, D, D], Rest4, TempWord),
    % The remaining letters must be unique and not 'A', 'B', 'C', 'D'
    length(TempWord, TempLength),
    ExtraLength is N - TempLength,
    findall(X, (member(X, Rest4), X \= A, X \= B, X \= C, X \= D), UniqueLetters),
    length(UniqueLetters, UniqueLength),
    (ExtraLength =< UniqueLength ->
        length(ExtraLetters, ExtraLength),
        append(ExtraLetters, _, UniqueLetters),
        append(TempWord, ExtraLetters, FullWord),
        permutation(FullWord, Word)
    ; fail).
...
% write_words_to_file(+Predicate:atom, +Alphabet:list, +N:integer, +Filename:atom)
% Writes all words of length N generated by Predicate from Alphabet to the specified Filename.
write_words_to_file(Predicate, Alphabet, N, Filename) :-
    open(Filename, write, Stream),
    forall(call(Predicate, N, Alphabet, Word), (write(Stream, Word), nl(Stream))),
    close(Stream).

% write_all_words_to_files
% Writes all words of length 5 with 2 'a's, words of length 6 with 2 letters repeated twice,
% and words of length N with 4 letters repeated twice to files.
write_all_words_to_files :-
...
%    N = 10, % Example word length, can be changed
%    write_words_to_file(words_with_4_repeated_2, Alphabet, N, 'data/words_n.txt').
